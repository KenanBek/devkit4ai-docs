...
**Structure:** `dk_` + 32 URL-safe characters
...
**HTTP header:**
```bash
X-Developer-Key: dk_abc123XYZ-_789def456ghi012jkl345
```
...
**Structure:** `ak_` + 32 alphanumeric characters
...
- 32 characters: random alphanumeric string
...
<Note>
The environment variable name uses `PROJECT_KEY` for clarity, but the actual key prefix is `ak_` (not `pk_`).
</Note>

**HTTP header:**
```bash
X-API-Key: ak_abc123XYZ789def456ghi012jkl345mn
```
...
| Feature | Developer Key (`dk_`) | API Key (`ak_`) |
|---------|----------------------|-----------------|
| **Purpose** | Developer console access | End-user project access |
| **Prefix** | `dk_` | `ak_` |
| **Character set** | Alphanumeric + `-` + `_` | Alphanumeric only |
...
**Full key (shown once):**
```
dk_abc123XYZ-_789def456ghi012jkl345
```
...
```typescript
function isValidDeveloperKey(key: string): boolean {
  return /^dk_[a-zA-Z0-9_-]{32}$/.test(key)
}
...
// Usage
const devKey = 'dk_abc123XYZ-_789def456ghi012jkl345'
console.log(isValidDeveloperKey(devKey)) // true

const apiKey = 'ak_abc123XYZ789def456ghi012jkl345mn'
console.log(isValidApiKey(apiKey)) // true
```
